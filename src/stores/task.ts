...
export const useTaskStore = defineStore('task', () => {
  ...
  const filteredTasks = computed(() => {
    ...
    if (filter.value.dateTo) {
      result = result.filter((t) => t.createdAt <= filter.value.dateTo! + ' 23:59:59')
    }
    ...
    result.sort((a, b) => {
      ...
      switch (field) {
        case 'priority': {
          const weightA = PRIORITIES.find((p) => p.value === a.priority)?.weight ?? 0
          const weightB = PRIORITIES.find((p) => p.value === b.priority)?.weight ?? 0
