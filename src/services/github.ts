  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      if (attempt < 3) {
        ...
        // 后台同步的网络波动不应污染控制台，用 debug 级别
        console.debug(
          `[GitHub] ${label} 失败（第 ${attempt} 次），${delay}ms 后重试...`
        );
 ...
 * @param month 月份 (YYYY-MM)
 */
export async function pullReflections(
  month: string
): Promise<SyncResult & { reflections?: Reflection[] }> {
  if (!isOnline()) {
    return { success: false, message: "当前离线，无法同步" };
  }
  ...
  try {
    ...
    if ("content" in response.data) {
      ...
      return {
        success: true,
        message: `成功拉取 ${reflections.length} 条反思`,
        reflections,
      };
    }
...
export async function pullSessions(
  month: string
): Promise<SyncResult & { sessions?: Session[] }> {
  if (!isOnline()) {
    return { success: false, message: "当前离线，无法同步" };
  }
  ...
  try {
    ...
    if ("content" in response.data) {
      ...
      return {
        success: true,
        message: `成功拉取 ${sessions.length} 条会话`,
        sessions,
      };
    }
...
export async function pushEvent(event: OutboxEvent): Promise<SyncResult> {
  ...
  try {
    ...
    if (
      ...
      return {
        success: true,
        message: `事件 ${event.eventId} 已存在（幂等跳过）`,
      };
    }
