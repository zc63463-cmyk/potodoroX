...
vi.mock('./outbox', () => ({
  ...
  markTombstone: (t: string, e: string, ts?: string) => mockOutbox.markTombstone(t, e, ts),
...
// 导入被测模块
const { consumeEvents, parseTime, isNewerThan } = await import('./event-consumer')
...
describe('isNewerThan', () => {
  it('本地晚于事件 → true', () => {
    expect(isNewerThan('2026-05-05 00:00:00', '2026-05-03T00:00:00.000Z')).toBe(true)
  })
  it('本地早于事件 → false', () => {
    expect(isNewerThan('2026-05-03 00:00:00', '2026-05-05T00:00:00.000Z')).toBe(false)
  })
  it('相同时刻 → false', () => {
    expect(isNewerThan('2026-05-04 10:00:00', '2026-05-04T10:00:00.000Z')).toBe(false)
  })
...
describe('consumeEvents', () => {
  ...
  it('正常消费 task.created', async () => {
    const e = makeEvent({
      payload: { id: 'task-1', title: '新建', updatedAt: '2026-05-04 10:00:00' },
    })
  ...
  it('旧 task.updated 不覆盖本地新数据', async () => {
    ...
    mockDb.getTask.mockResolvedValue({
      id: 'task-1', title: '新版本', updatedAt: '2026-05-04 17:00:00',
    } as Task)
    const old = makeEvent({
      type: 'task.updated',
      timestamp: '2026-05-04T08:00:00.000Z',
      payload: { id: 'task-1', title: '旧版本', updatedAt: '2026-05-04 08:00:00' },
    })
  ...
  it('task.deleted 后旧 task.created 不复活', async () => {
    const del = makeEvent({
      type: 'task.deleted', timestamp: '2026-05-04T10:00:00.000Z',
      payload: { id: 'task-1' },
    })
    const oldCreate = makeEvent({
      type: 'task.created', eventId: 'old-create', timestamp: '2026-05-04T09:00:00.000Z',
      payload: { id: 'task-1', title: '旧创建', updatedAt: '2026-05-04 09:00:00' },
    })
    ...
    mockOutbox.getTombstone.mockResolvedValueOnce({
      entityId: 'task-1', entityType: 'task', deletedAt: '2026-05-04T10:00:00.000Z',
    })
    ...
    expect(mockOutbox.markTombstone).toHaveBeenCalledWith('task', 'task-1', '2026-05-04T10:00:00.000Z')
  ...
  it('删除后新 task.created 可恢复实体', async () => {
    const del = makeEvent({
      type: 'task.deleted', timestamp: '2026-05-04T09:00:00.000Z',
      payload: { id: 'task-1' },
    })
    const newCreate = makeEvent({
      type: 'task.created', eventId: 'new-create', timestamp: '2026-05-04T10:00:00.000Z',
      payload: { id: 'task-1', title: '新创建', updatedAt: '2026-05-04 10:00:00' },
    })
    mockGithub.pullEvents.mockResolvedValue([del, newCreate])
    // .deleted 不走 getTombstone，只需为 newCreate 配置 1 次
    mockOutbox.getTombstone.mockResolvedValueOnce({
      entityId: 'task-1', entityType: 'task', deletedAt: '2026-05-04T09:00:00.000Z',
    })
    const r = await consumeEvents()
    expect(r.processed).toBe(2)
    expect(mockDb.deleteTask).toHaveBeenCalledWith('task-1')
    expect(mockOutbox.markTombstone).toHaveBeenCalledWith('task', 'task-1', '2026-05-04T09:00:00.000Z')
