...
import {
  parseImportFile,
  type ImportReflection,
} from "@/utils/importReflection";
...
const syncDateEnabled = ref(false);
const syncDateStart = ref("");
const syncDateEnd = ref("");
...
async function syncWebDav() {
  ...
  // 加载各类型数据
  const dataMap: { reflections?: any[]; sessions?: any[]; tasks?: any[] } = {};

  if (syncTypes.value.includes("reflections")) {
    await reflectionStore.loadReflections();
    dataMap.reflections = reflectionStore.reflections;
  }
  if (syncTypes.value.includes("sessions")) {
    await sessionStore.loadAllSessions();
    dataMap.sessions = sessionStore.sessions;
  }
  if (syncTypes.value.includes("tasks")) {
    await taskStore.loadTasks();
    dataMap.tasks = taskStore.tasks;
  }

  const dateRange =
    syncDateEnabled.value && syncDateStart.value && syncDateEnd.value
      ? { start: syncDateStart.value, end: syncDateEnd.value }
      : undefined;

  const result = await webDav.sync(dataMap, {
    types: syncTypes.value,
    dateRange,
  });
          ...
          <div class="form-group">
            <div class="toggle-row">
              <div class="toggle-info">
                <span class="toggle-label">按日期范围同步</span>
                <span class="toggle-desc">仅同步指定日期范围内的数据</span>
              </div>
              <button
                class="toggle-switch"
                :class="{ active: syncDateEnabled }"
                @click="syncDateEnabled = !syncDateEnabled"
              >
                <span class="toggle-knob" />
              </button>
            </div>
            <div v-if="syncDateEnabled" class="date-range-row">
              <input
                type="date"
                v-model="syncDateStart"
